...
#Recording audio data as csv
record_a = pd.DataFrame(columns=list('ABC')) 
...
def recorder_callback(in_data, frame_count, time_info, status_flags):
    ...
    if rms > 0.1:
    	print rms, current_time
            
    
	print "Level exceeded!!!"

    ##Recording audio data
    record_a.loc[len(record_a)] = [datetime.datetime.now(), current_time, rms]
...
record_agg = record_a.RMS.resample('1S', how=(np.mean))

#convert to decibels
decibels = pd.DataFrame({'Time': record_agg.index, 'RMS': record_agg.values})
p_0 = min(record_a.RMS) #Setting lowest recorded sound level as reference 
